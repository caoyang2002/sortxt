...
fn build_char_pool() -> Vec<char> {
    ...
    let chinese_common = vec![
        ...
        '说', '去', '就', '得', '也', '子', '你', '那', '要', '和', '看', '大', '而', '中', '地', '对',
        '生', '能', '成', '第', '分', '多', '好', '自', '其', '么', '下', '天', '时', '过', '着', '出',
        '会', '以', '可', '之', '后', '于', '年', '发', '都', '然', '同', '行', '前', '所', '两', '种',
        '事', '样', '家', '现', '当', '想', '并', '已', '开', '动', '从', '面', '又', '知', '理', '体',
        '进', '实', '些', '主', '本', '相', '等', '点', '合', '意', '信', '务', '变', '几', '条', '性',
        '别', '给', '她', '由', '问', '加', '更', '全', '部', '明', '力', '量', '件', '金', '位', '次',
        '表', '话', '水', '化', '名', '高', '文', '反', '身', '义', '革', '立', '基', '工', '作', '民',
        '经', '原', '动', '如', '产', '长', '把', '党', '法', '学', '没', '还', '心', '公', '主', '机',
        '组', '结', '果', '月', '日', '电', '使',
    ];
...
fn random_string(rng: &mut impl Rng, min_len: usize, max_len: usize) -> String {
    ...
    (0..len)
        .map(|_| *CHAR_POOL.choose(rng).unwrap())
        .collect()
}
